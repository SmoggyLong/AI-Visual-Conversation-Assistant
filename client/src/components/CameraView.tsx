 ...
 * 摄像头预览组件。
 *
 * video 元素始终存在于 DOM 中（通过 CSS 控制显隐），
 * 确保 videoRef 在任何时刻都可用，避免 stream 绑定失败。
 */
export function CameraView({ camera }: CameraViewProps) {
  return (
    <div className="flex-1 relative bg-black overflow-hidden">
      {/* video 始终渲染，通过 CSS 控制可见性 */}
      <video
        ref={camera.videoRef}
        autoPlay
        playsInline
        muted
        className={`w-full h-full object-cover ${camera.state.enabled && !camera.state.error ? '' : 'hidden'}`}
      />

      {/* 未开启占位 */}
      {!camera.state.enabled && !camera.state.error && (
      ...
      {/* 分辨率标签 */}
      {camera.state.enabled && camera.state.resolution && (
